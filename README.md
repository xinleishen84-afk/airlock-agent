...
- **机构名前截断只补得回一部分。** 方向与地址相反：地址丢的是后面（可以向后
  拉伸补），机构名丢的是前面。中文与拉丁文各有一套边界规则，都已实现——
  中文靠停止字（`我在星辰科技有限公司` 检出 `科技有限公司` 能补回 `星辰`），
  拉丁文靠首字母大写加停止词表（`Acme Global Ltd` 检出 `Ltd` 能补回全名，
  而 `On Monday Acme Ltd` 不会把 `On Monday` 吞进去）。

  **补不回的是没有任何标志词的字号**：`阿里巴巴集团` 检出 `集团` 之后，
  `阿里巴巴` 既不含行业修饰词也不含行政区划，邻近度评分为 0 被拒——
  向左强行扩只会把动词和主语一起吞进来。这一类需要名册或更大的模型。
- **按文字系统路由只是按语言路由的粗略代理。** 德语 `Steuer`、西班牙语
  `registrado` 送进英文模型同样是分布外输入，实测造成 3 处过度脱敏。
...
| | |
|---|---|
| 测试 | Go 572 个测试函数 / 20,731 行测试代码 / 主模块覆盖率 74.1% / `-race` 全绿；Python 21 个 |
...
| CI | 六个 job（四个 Go 模块 + 配置自检 + Python 分析器）在 GitHub Actions 上全绿。前三次运行暴露了四处只在 Linux 干净环境下才出现的问题，见「踩过的坑」 |
| **零外部验证** | 没有任何人在生产里跑过它 |
...
./airlock \
  ...
  --session-consistency single-replica \
  --name-roster ./names.txt \
  --audit-sink /var/log/airlock/audit.jsonl \
  --audit-key-file /var/run/secrets/audit-key \
  --ner http://ner-service:8000/v1/detect   # 不配 --ner 时，姓名类召回率为 0

# 不配 --audit-sink 是合法选择，但那样一条 GDPR 审计事件都不会发出。
# 启动日志会明确说轨迹是开还是关——三个二进制（sidecar / advanced / 网关）
# 用的是同一套 flag 与同一份装配实现。
```
...
路由算出来的语言是**逐次调用**送到线上的，不是客户端的全局默认值。
这一点值得写下来，因为它曾经不成立：函数签名收了 `language` 参数，
wire 请求里发的却是 `c.opts.Language`，整条路由链在最后一跳作废而没有任何
症状。现在有 wire 契约测试钉住它，见「踩过的坑」。

按文字系统路由只是**按语言路由的粗略代理**：德语与西班牙语同样会落进英文
模型，那对它们也是分布外。契约里的 `language` 字段已经支持真正的语言识别，
缺的是识别器本身。
...
| `CacheTokenStore` | 多副本，**且后端满足下列硬要求** | ✅ | ✅ |
| `SQLTokenStore` | 加密关系库 | ✅ | ✅ |
...
**`CacheTokenStore` 不是「任意 KV 都能接」。** 这一行此前写的是
「Redis / DynamoDB / 任意 KV」，那句话给错了预期。后端必须同时满足：

| 要求 | 不满足会怎样 |
|---|---|
| `FindOrCreate` 原子且线性化 | 并发签发产生两个令牌，同一个人在下游被当成两个实体 |
| TTL 不滑动（命中不续期） | 被反复提到的 PII 永不过期，保留期在事实上被取消 |
| 前缀删除彻底 | GDPR 第 17 条的擦除留下残余 |
| **正常运行不静默丢映射** | 已经发给模型的令牌永远还不回去，且没有任何错误提示 |

最后一条把大多数「缓存」排除在外：**Memcached、以及配了 `allkeys-lru` 的
Redis 会在内存压力下驱逐任意键**，而令牌映射是恢复原值的**唯一**来源。
生产上 Redis 必须 `maxmemory-policy noeviction` 并做容量监控——
`noeviction` 下内存打满表现为写入报错，那是需要在到达之前就看见的信号。

单条命令做到 `FindOrCreate` 需要 Redis **7.0** 起的 `SET key val NX GET`；
更早版本用 Lua 或 Redis Function。本设计每条映射只写一个键，
因此 Redis Cluster 下不涉及跨槽（CROSSSLOT）问题。

**这个库存的不是缓存，是 PII 映射数据库，只是恰好放在一个 KV 里。**
逐条要求与理由写在 `pii/anonymize` 里 `Cache` 接口的注释上。

`SQLTokenStore` 的值用 `K_data_vN` 做 AES-GCM，行内记 `key_version`，
AAD 绑定租户、命名空间、令牌与版本——一行密文被挪到另一个租户名下、
或换到另一个令牌下，都会**解不开**。
...
**两个后端的令牌来源不同,不可关联性的依据也不同。**
`SQLTokenStore` 的令牌是随机生成后存下来的,随机性本身就给出跨租户不可关联;
`CacheTokenStore` 的令牌是确定性派生的,随机性没了,**不可关联完全落在密钥上**
——同一个手机号在两个租户下算出不同令牌,靠的是两把在计算上不可关联的租户
密钥。这句话此前写的是「令牌是随机的,本就不可关联」,那对派生式令牌不成立。

### 令牌身份与数据加密是两套密钥

```
租户密钥层级
   ├── K_token         只用于 HMAC 派生令牌身份
   └── K_data_vN       只用于 AEAD 加解密 PII,可版本化轮换
```

分开的直接后果:**数据密钥轮换不改变令牌身份**。合在一起时这两件事是同一个
动作——想换加密密钥就得接受所有历史令牌全部改变,不想改令牌就永远不能换
加密密钥。

令牌身份的稳定性是**有边界的承诺**:它在一个身份 epoch 内稳定,而根密钥轮换
即开启新 epoch。承诺「令牌永远稳定」做不到,因为身份是从根密钥派生的。
日常的数据密钥轮换不触碰它,只有根密钥轮换才会。

**缓存里没有明文。** 记录形如
`atr1.<密钥版本>.<身份epoch>.<nonce>.<密文>.<到期时刻>`,AAD 绑定租户、
命名空间、令牌、密钥版本与身份 epoch——把一份密文搬到另一个令牌下会认证失败。
`CacheTokenStore` 是一个 PII 映射数据库,只是恰好放在 KV 里,不是普通缓存:
它被驱逐一条,就有一个已经发给模型的令牌永远还不回去。生产上必须
`maxmemory-policy noeviction`,详见 [SECURITY.md](SECURITY.md) 与
`pii/anonymize` 里 `Cache` 接口的注释。
...
### NER 不可用时：fail-closed 还是 fail-open

这是一个真的开关，不是一句话——两种语义的行为确实不同，各自的代价写在下面。

| | 行为 | 代价 |
|---|---|---|
| **fail-closed**（默认） | 阻断请求 | 模型挂了，业务也停 |
| **fail-open** | 降级为前两层（正则、校验位、名册、复姓） | **姓名、地址、机构名本次完全未检测**，请求照常出境 |

职责是分开的:传输层只报告故障类别并标出「这一类可降级」,它知道自己被配成
fail-open,但**不知道这次请求能不能少了模型结果还继续——那是安全策略,不该
由一个管网络通信的组件拿主意**。级联层决定拿这个标记做什么,它也是唯一持有
前两层结果的地方。

降级次数单独计数(`CascadeStats.ModelDegraded`),不与「因无触发特征而跳过模型」
混在一起:跳过是「这段文本不值得送模型」,降级是「该送但送不了」。混在一起,
一次长时间宕机会表现为触发率下降,看起来像流量特征变了。
...
**接口停在原语级，正确性就得每个调用方各写一遍。** `Cache` 原来暴露的是
`Get`/`SetNX`。`SetNX` 回答的是「我存进去了吗」，而调用方要的是「这个键上
权威的值是什么」——于是每个调用方都得在 `SetNX` 返回 false 之后再补一次
`Get`，而那次补读不在同一个原子步骤里：两次调用之间键可能过期，于是每个
调用方还得各自处理「刚被拒绝、现在又不存在了」。把「查—建—取回」压成一个
`FindOrCreate`，是把正确性的负担从每一个调用方移到唯一的实现方。**组合处
正是撕裂发生的地方，而组合是每个调用方各写一遍的。**

契约本身也写成了可执行的一致性测试：注释里的「原子」拦不住一个先查后建的
实现——它单线程下完全正确，只在并发下偶尔多创建一次。第一版用例还真就没
抓住这种实现:协程是循环里逐个起的，第一个走完那个间隙时后面的还没被调度。
**窗口窄的时候并发度救不了，靠重复**——现在跑 200 轮，一轮翻车就算抓住，
拿故意写坏的 check-then-set 实现验过，第 0 轮就红。

**排不出一个安全的写入顺序时，说明不该有两次写。** `CacheTokenStore.Issue`
原来写两个键：正向「值摘要 → 令牌」与反向「令牌 → 值」。先写正向时，反向写
失败会留下一条指向空处的正向记录——下一次同一个值走快路径直接命中，**成功
返回**一个还原不了的令牌，且每次调用都返回同一个坏令牌直到 TTL 到期。坏的
不是失败的那一次调用，是之后每一次。

调换顺序能堵住写失败那条边，再给反向 TTL 加余量能堵住过期那条边。但缓存在
`maxmemory` 压力下独立驱逐任一键这条边，两者都盖不住——**顺序与 TTL 只能把
可观察到撕裂的窗口挪小，消不掉**。而 `Cache` 接口只有 `Get`/`SetNX`/
`DeleteByPrefix`，加一个跨键事务原语会逼每个实现都提供跨键原子性，做不到的
只能假装。

真正的解法是消掉第二个键：令牌改为 `HMAC(租户密钥, 命名空间‖原值)` 确定性
派生，同一个值算出同一个令牌这件事由函数保证，不需要存储去记。正向索引因此
整个不存在，只剩一个键、一次写——**没有第二个键，就没有可撕裂的中间态**，
并发各方也不再是竞争一个赢家，而是由构造方式直接达成一致。

顺带堵掉一个比撕裂更早的弱点：原来的正向索引键是不加密钥的
`sha256(租户‖命名空间‖原值)`，任何拿到缓存读权限的人猜一个手机号、算一遍、
看键在不在，就能证实它在不在系统里——一次不需要解密任何东西的存在性泄露。

**函数收了参数却不用它，测试测的是参数算得对不对。**
`DetectContextLanguage(ctx, text, language)` 的 wire 请求里写的是
`c.opts.Language`——签名收了 `language`，注释还写着「cascade overrides this
per call」，而线上一律发客户端默认值。整条语言路由链在最后一跳作废：Cascade
按文字系统算出 zh / en 逐段传下来，服务端收到的永远是同一个值。症状是安静的
——请求成功、实体也检出来了，只是拉丁文段落进了中文模型。上游单测测的是
`ScriptOf()` 算得对不对，它一直算得很对；**缺的是「算出来的有没有被用上」**。
补法是 wire 契约测试：用假 stub 断言线上实收的值，并刻意让客户端默认值与
per-call 值不同——两者相同的话，这个 bug 测不出来。

**一个只改错误文案的开关。** `FailOpen=true` 时客户端返回的错误里写着
「已按 fail-open 放行」，而级联层收到 error 直接 `return nil, err`。配了
fail-open 的部署在 NER 挂掉时与 fail-closed 表现完全一样——开关等于不存在，
而它的名字让人以为存在。修法不是把 error 换成 nil：那样调用方无法区分
「没检出实体」与「模型根本没跑」。传输层只标记故障类别（它知道自己被配成
fail-open，但**不知道这次请求能不能少了模型结果还继续——那是安全策略**），
级联层决定，它也是唯一持有前两层结果的地方。

**降级与跳过必须分开计数。** 跳过是「这段文本不值得送模型」，降级是「该送
但送不了」。混在一个计数里，一次长时间宕机会表现为触发率下降，看起来像
流量特征变了。

**只在 macOS 上跑过的东西，第一次进 Linux CI 会掉四样。**
`analyzer/Dockerfile` 里 `VAR=x for ...; do` 是非法 POSIX sh（环境变量前缀
只能加在简单命令前），这一行写下来就提交了、从没被构建过；`-race` 下的延迟
用例在共享 runner 上撞 300 秒超时，而它测出来的 P99 量的是竞态检测器不是
本系统；测试夹具把时长写成裸整数 `int64(time.Hour)`，配置解析器刻意拒绝
裸整数（`300000000` 既可能是纳秒也可能是毫秒）；以及一个改完时长格式后
没人用的 `time` 导入，让整个集成模块编译不过。

**测试替身比被测系统更宽松，就是在测一个不存在的世界。** 把 `SQLTokenStore`
接进统一契约套件后立刻暴露一个真缺陷：`SELECT` 与 `INSERT` 之间有间隙，两个
副本会同时落空、同时插入，而 `UNIQUE` 约束拒绝后到的那个——代码把这个拒绝
当成写入故障抛出，**真库语义下 32 次并发签发有 4 次直接失败**，输家没有采纳
赢家。它此前从未暴露，是因为假 DB 用一把全局锁把两步串起来，**比真数据库更
原子**，冲突从未发生。同类的还有假缓存曾经完全忽略 TTL 参数，于是一整类
TTL 不变量在测试里不可观测。替身比生产更严格是安全的，更宽松则不是。

**冗余的防护要说成冗余。** 令牌记录的 AAD 绑了五项，实测把 `tenant` 或
`key_version` 拿掉，**没有任何用例变红**——数据密钥本就按这两项派生，跨租户
或跨版本的密文在拿到 AAD 之前已经解不开。真正承重的是 `namespace`、`token`
与 `epoch`。两项冗余仍然保留作纵深，但注释按实测结果写：把冗余说成承重，
会让后来人以为动密钥派生是安全的。
