...
**两键写撕裂时，坏的不是失败的那一次调用。** `CacheTokenStore.Issue` 要写
两条记录：正向「值摘要 → 令牌」与反向「令牌 → 值」。原来先写正向——反向写
失败时 Issue 报错，看起来是安全的，但正向那条留下了。下一次同一个值进来走
快路径直接命中，**成功返回**那个令牌，而它没有反向记录：令牌进入出站载荷，
模型带回来后 `Resolve` 返回 `ok=false`，被当成模型捏造的幻影，真值永远还不
回去。且每一次针对该值的调用都返回同一个坏令牌，直到 TTL 到期。

改成反向先写就没有这个状态：反向失败什么都不留，正向失败只留一条没人指向
的反向记录，是垃圾但无害。**但只调换顺序不够**——两次写差着几毫秒，同样的
TTL 意味着先写的先过期，反向先写就先过期，于是又会有一小段时间快路径返回
的令牌已经还原不了。这正是原来那个顺序碰巧避开的边。所以反向 TTL 要加一个
余量，让正向总是先过期；余量是常量而不是 TTL 的比例，因为反向记录里存的是
明文原值，余量有多长 PII 就多留多久。
